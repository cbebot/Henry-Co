...
## 10. Feature program (owner directive 2026-07-11 — "smart features + remarkable things")

An external enhancement report (competitor-benchmark style: Shopify/Wix/Squarespace/
WordPress) was reviewed against the live codebase. Verdict per item:

**Already built — the report under-estimates the platform (do NOT rebuild):**
- "Commerce & billing integration (Stripe)" → a provider-routed money spine with a
  balanced double-entry ledger, VAT engine, wallet rails and live Flutterwave already
  runs the company. The owner console READS it; nothing to integrate.
- "Roles & permissions" → staff invite/role/permission CRUD + audit trail exist.
- "Content management" → brand/site/pages/people CRUD feeds the LIVE public hub.
- "Dark mode / theming" → OCC-1 makes both themes real (light debuts with this program).
- "Localization" → typed copy modules across 11 locales; the ~14 hardcoded-EN owner
  pages are queued in OCC-5.
- "AI insights (long-term)" → the Founder Intelligence system (F2 real assistant, F3
  governed actions) is built and flag-dark — years ahead of the report's roadmap.
- "Onboarding tours/tooltips" → consumer-SaaS pattern, skipped deliberately: the
  founder AI is the help layer, and metric PROVENANCE (trace drawers) is the serious
  version of a tooltip. Voice rule: no hand-holdy walkthroughs, no exclamation copy.
- "Mobile app" → out of scope by owner decision; the shell is responsive.
- "A/B testing" → real infra spend for one owner pre-launch; noted long-term, out of
  this program. "GA4/Matomo/SendGrid/gettext" suggestions are superseded by house
  infra (henry_events, SES, typed copy modules) — external vendors are not named in
  product anyway.

**Adopted from the report (genuinely missing):**
- R1. Real-time dashboard — overview KPIs update in place via the EXISTING
  OwnerRealtimeBridge subscription (a sale lands → the money strip ticks). → OCC-3.
- R2. Trends — day-series on overview (revenue from ledger FlowPoint, signups series,
  7-day division sparklines via the existing Sparkline component). → OCC-3.
- R3. Metric drill-downs — every KPI clickable to its rows (§4 detail views). → OCC-4.
- R4. Alert lifecycle + routing — signals gain acknowledge / snooze / resolve state
  (new deny-RLS table, guarded route, audited) so pressure is a workable queue, not
  the same wall every visit; per-severity email/WhatsApp routing preferences. → OCC-5.
- R5. Empty states + decision-point microcopy everywhere (calm authority, no "!"). →
  rebuild standard, all slices.

**Original additions (the "remarkable" tier — each grounded in existing infra):**
- W1. "Since you last looked" — a per-owner last-seen marker; the overview greets with
  the DELTA (revenue moved, signups, signals cleared/new) since the previous visit.
- W2. Morning brief on the dashboard — the F2b narrative ("This morning, in brief")
  renders at the top of /owner when the founder flag is live; email keeps parity.
- W3. Decision queue with governed one-tap actions — approvals page becomes a live
  queue where each item carries its F3 confirmation card as tranches land (see →
  decide → act → audited, without leaving the command center). Phase F surfaces here.
- W4. Provenance everywhere — every number gets a registered trace (SQL + row sample
  drawer) + shareable /owner/trace/[id]. "No number without receipts."
- W5. Board pack surface — the weekly/monthly branded PDFs ALREADY generated by the
  reporting crons (owner-reports bucket) get signed-URL download cards in finance +
  overview; today they are email-only.
- W6. Anomaly-aware signals v1 — deterministic baseline comparison (trailing same-
  weekday median) so signals can say "care bookings at 2.7× the usual Friday volume";
  no fake AI, feeds the founder facts pack too.
- W7. Keyboard-first — fix the broken palette commands source, register per-module
  actions + inbox/v3-launch modules, ⌘J summons the founder surface (OCC-2).
- W8. Escalation continuity — AI conversation transcripts link straight into the
  escalated support thread (§4 item 1).

Phase mapping: R1/R2/W1/W2/W5/W6 → OCC-3 · R3/W4/W8 → OCC-4 · R4/W3/W7 → OCC-2/5/F.
