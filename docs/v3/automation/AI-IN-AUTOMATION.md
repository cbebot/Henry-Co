...
- There is **no general-purpose automation debit RPC** on main. The wallet-debit paths — metered-AI charge (`post_ai_usage_charge`, hard-capped at a *customer-authorized prepaid hold*), the withdrawal reserve (*user-initiated*), and the refund clawback (`initiate_payment_refund`, reverses a customer's *own* top-up) — are each `SECURITY DEFINER`, `service_role`-only, and **each debits only against a user-initiated anchor row** (a prepaid AI hold, a `pending_review` withdrawal request, or a refund of the customer's own funding request). _Re-grounded 2026-07-24 for precision:_ because they are `service_role`-granted, a service-role automation *can* technically invoke them — so the guarantee is not DB-level unreachability but **the app-layer design rule (below) + the anchor-row requirement**: none is a general-purpose debit, and Phase F invokes none.
