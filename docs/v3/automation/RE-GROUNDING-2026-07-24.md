...
| **Internal AI-spend ledger** | studio daily-ceiling read `dailyAgencySpendKobo(now)` (sums `studio_build_jobs.budget/cost` over the UTC day) — a *derived* counter | `ai_operator_spend_ledger` (window_day PK + `ai_operator_spend_today/add` RPCs, reserve-before-run, degrade-CLOSED) — a *durable* counter, cloned from `ai_free_spend_ledger` | ONE internal-spend pattern: the shipped `ai_free_spend_ledger` (`20260705150000`, **live on prod**). The operator's clone (`ai_operator_spend_ledger`) is a **third** counter — reconcile it into the shared pattern rather than let Phase F add a fourth (see [AI-IN-AUTOMATION.md](./AI-IN-AUTOMATION.md) §3). |
