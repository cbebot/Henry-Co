...
**Pass:** V3-F-DESIGN-01 · Design only. Deliverable 2. The brief requires this specced as **testable CI rules, not prose**. Each rule below has an id, a machine-checkable statement, a detection method, pass/fail criteria, and the failure it prevents. All symbols are grounded on `origin/main @ 8c9794b5`.
