...
**Pass:** V3-F-DESIGN-01 (refresh) · **Design only — no feature code, no migration.** · **Re-grounded onto `origin/main @ 241f068a` (2026-07-24), superseding the `8c9794b5` base of PR #517.** Stacks on nothing.

> Every load-bearing claim was originally grounded on `origin/main @ 8c9794b5` (PR #517, 13-agent sweep + adversarial pass) and is **re-grounded onto `241f068a`** here — SA-2/SA-3/SA-4 have since merged, so the [RE-GROUNDING-2026-07-24.md](./RE-GROUNDING-2026-07-24.md) file is authoritative where the two disagree. Line numbers drift; trust the named symbols. Live-DB facts are tagged **PROD-ACTUAL** and re-probed 2026-07-24 (see the re-grounding §4).
...
3. the **studio-agency orchestrator** — _re-grounded 2026-07-24:_ **SA-1/SA-2/SA-3/SA-4 are now all MERGED** (`#511`/`#512`/`#523`/`#524`); the SA-2/SA-3 tables + tick + decisions inbox + `approved_artifact_hash` are **applied on prod**, and SA-4 shipped a **second** operator drain loop + lock + spend ledger (code merged, migration not yet applied). The engine-unification thesis is unchanged; the migration is now a retrofit of shipped code ([RE-GROUNDING](./RE-GROUNDING-2026-07-24.md)).
