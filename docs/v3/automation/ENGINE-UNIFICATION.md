...
1. **CAS-claim helper.** _Re-grounded 2026-07-24 — two mechanisms, don't conflate them:_ SA-3's **shipped** tick claim (`claimJob`, `apps/studio/lib/agency/store.ts`) is a **`WHERE claimed_by IS NULL` conditional-UPDATE CAS** (no row lock; single-worker-per-job by the atomic conditional update), and the shipped outbox drain relies on single-flight, not a row lock. `SELECT … FOR UPDATE SKIP LOCKED` is **not shipped anywhere** (grep = 0 outside docs) — it is the pattern the rail's *proposed* `claim_workflow_jobs` RPC will standardize to. The rail unifies BOTH into one claim primitive; the point is one convention, whichever lock mechanism it lands on.
...
- SA-3 uses the **same conventions** the rail will standardize: the `claimed_by IS NULL` conditional-UPDATE CAS (shipped; the rail may promote it to `FOR UPDATE SKIP LOCKED` in the `claim_workflow_jobs` RPC), the `owner-inbound-email` HMAC handshake, `(jobId, attempt)` idempotency, `henry.studio.build.*` names in the canonical union, `writeAuditLog(correlationId = jobId)`.
