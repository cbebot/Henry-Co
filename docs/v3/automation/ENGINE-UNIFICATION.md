...
> **Verification status:** the load-bearing premise here — "SA-3 is design-only on main; the substrate it names (`search_index_outbox` + minute-cron + `founder_action_proposals` + `owner-inbound-email` HMAC) is real" — was adversarially re-derived from primary sources and held. See [REGROUNDING-LEDGER §4](./REGROUNDING-LEDGER.md#4-verification-adversarial-rounds).
...
## The "two competing engines" risk is real — and prospective

Because **SA-3 is not built yet** ([REGROUNDING-LEDGER H2](./REGROUNDING-LEDGER.md#1-the-headline-corrections-read-these-first)), the risk is a *future* fork, not a *present* one — which means we can prevent it at design time rather than pay a retrofit later. If V3-43 and SA-3 are built independently, the estate ends up with:
...
> **The platform workflow engine GENERALIZES SA-3 — by promoting the existing `search_index_outbox`/`drainOutbox` idiom into one shared durable-job rail, on which SA-3's studio build lifecycle runs as a registered domain saga (not a second engine, not a flattened queue). Because SA-3 is design-only on main, the seam is defined prospectively; the domain-state boundary is documented above.**
