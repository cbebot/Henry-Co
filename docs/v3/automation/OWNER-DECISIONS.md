...
- **✦ Default — Generalize at the rail; SA-3 is a domain saga on the shared rail.** One durable-job rail (`@henryco/workflow`), promoted from the existing `search_index_outbox`/`drainOutbox` idiom; SA-3's studio build lifecycle keeps its own state table but runs its tick + side-effects **as registered handlers/jobs on the rail** — one drain loop, one retry/dead-letter/idempotency contract, not two engines. Because SA-3 is design-only today, add a one-paragraph reconciliation note to `docs/v3/studio-agency/PHASED-PLAN.md §SA-3` so it lands on the rail rather than building `/api/agency/tick`.
- **Variant:** keep them fully separate with a documented boundary. **Not recommended** — it ships two competing drain loops/CAS/retry conventions, the exact thing the brief forbids, for no gain (they already share a substrate).
- **What only you decide:** whether to accept the small coordination cost now (author SA-3 against the rail) to avoid a retrofit later. Recommendation: **yes.**
