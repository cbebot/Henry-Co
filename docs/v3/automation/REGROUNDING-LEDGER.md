# Re-Grounding Ledger — V3-43…48 claims vs real `origin/main @ 8c9794b5`
...
| **H2** | Memory/brief: "SA-3 orchestration state machine + decisions inbox (PR #513)" — treat as built. | **DESIGN-ONLY on main.** No `studio_build_jobs`/`studio_build_events`, no `/api/agency/tick`, no decisions inbox, no `owner.studio.*` actions, no `approved_artifact_hash` in merged code. Only **SA-1** (`brief-class.ts`, `releaseStudioProposal`, `20260718120000` migration) is merged. | The "SA-3 machine to generalize" is a **design contract** (`docs/v3/studio-agency/ARCHITECTURE.md §3`). The seam can be set **prospectively**. See [ENGINE-UNIFICATION](./ENGINE-UNIFICATION.md). |
