...
| **V3-FREESHIP-02** | freeship-seller-zones | **M** (money) + **pricing** | ⏳ backend built (T1–T4); UIs (T5/T6) next | **Seller-controlled, location-aware** free shipping ("Delivery Promises"): tier-earned reach (bronze=own state / silver=zone / gold=nationwide via KYC `verification_level`), store-wide. **Built + tested + committed-NOT-applied:** NG geography module (`@henryco/config`), tier-clamped reach (`delivery-reach.ts`), `marketplace_delivery_promises` table + ownership-gated RPC, location-aware checkout waiver (re-clamps to current tier — the money guarantee). **Remaining:** T5 seller settings card, T6 buyer state-picker + badge. Spec `docs/superpowers/specs/2026-06-19-marketplace-delivery-promises-design.md`; plan `docs/superpowers/plans/2026-06-20-marketplace-delivery-promises.md`. This pass IS the Lane-1 review activating CLOSE-01's dormant waiver. |
