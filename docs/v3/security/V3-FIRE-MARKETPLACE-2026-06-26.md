...
| **F-03** | Privilege escalation via email-only staff memberships + `getMarketplaceViewer` email-OR role binding | AuthZ / Priv-esc | **HIGH** |
...
| **F-06** | Anon reads price/stock/SKU of non-approved products (`product_variants` `USING(true)`) | Smart-leak | **MEDIUM** |
...
| **F-09** | Anon reads media of non-approved products (`product_media` `USING(true)`) | Smart-leak | **LOW–MED** |
...
### F-03 — Privilege escalation via email-only staff memberships + email-OR role binding (HIGH)
- **Category:** Broken authorization / privilege escalation. CVSS `AV:N/AC:H/PR:N/UI:N/S:U/C:H/I:H/A:H` ≈ **8.3** (AC:H because it requires controlling the seed mailbox). **Escalates to CRITICAL if `henrycogroup.com` is not actively controlled.**
...
- **Pre-fix reproduction (source + prod evidence):**
  ...
  - Prod: there are **7 active privileged membership rows with `user_id = NULL`**, all bound to a single email:
    ...
    ```
  - **Impact:** whoever authenticates a session with `…@henrycogroup.com` inherits owner+admin+finance+moderation+operations+support+vendor simultaneously → can verify payments and approve/release payouts. The seed email is on the **legacy `henrycogroup.com` domain** (the company is now Henry Onyx / henryonyx.com). If that domain lapses or is re-registerable, this is a full money-control account takeover.
...
Three company names coexist in the codebase: the package scope/codename `@henryco/*` and `marketplace_*`; the **customer-facing** strings "Henry & Co." (e.g. `route.ts:456,564,1741`); and the **legacy seed domain `henrycogroup.com`** (the F-03 staff email). The registered entity is **Henry Onyx Limited** (brand henryonyx.com). This is the RR-1 legal-name cleanup; the `henrycogroup.com` instance is load-bearing in the auth path (F-03), so it should be prioritized alongside the F-03 fix.

---

**V3-FIRE-MARKETPLACE COMPLETE — 15 findings (1 critical / 3 high / 6 med / 5 low; +1 branding note), each with pre/post-fix evidence; 10 fix migrations proposed for owner-gated apply; division audited every class + state×surface matrix. Live DB posture is sound (no RLS-off table, all IDOR probes closed, lockboxes deny, money consistent); residual risk is concentrated in the `service_role` app layer (F-01..F-05, F-10) and the email-OR role binding (F-03), neither of which has a DB backstop because no table is FORCE-RLS'd.**
