...
-- G10 (FIX-01): auth users with NO profiles row = exposure of the profiles
...
- **Catch-up:** the snapshot pre-dates later prod applies, so all 107 baseline files and the 13 `prod-actual/captured-migrations` were replayed in timestamp order (two tolerant passes). 15 baseline/captured files still fail after both passes. **9** are re-creations of objects the snapshot already holds (`policy … already exists`). The other **6** are early-era files that don't replay on a schema-only snapshot: `account_integration_hardening` (missing `display_name`), `hq_internal_comms_attachments_visibility_rls` (syntax near `[`), `data_governance_foundation` (VALUES length mismatch; this is why the `identity_account` row is missing in the shadow, the E13 artifact), `customer_lifecycle_snapshot` (FK), `user_addresses_legacy_backfill` (syntax near `do`), and `realtime_publication_backfill` (`rooms_messages` missing; FIX-01 guards it, §4). None is a candidate. **Correction (FIX-01, verifier C):** the tolerant second pass also *succeeded* in re-running three early captured migrations (`20260409054321/…054347/…054419`) **after** `sec_harden_02/03`. That re-created 28 world-open `"Service role full access" USING (true)` policies, so this shadow was less locked down than prod: the repo's own `sec_harden_03/04/04b` and `membership` grant invariants fail on it. DDL-apply results are unaffected, but RLS conclusions drawn on it were optimistic. §8.1 re-derives everything on a corrected shadow. Local shadow fixes: two missing `storage.buckets` columns (`file_size_limit`, `allowed_mime_types`) were added to the bootstrap stub.
...
- **RLS matrix (care/jobs, on the hardened shadow).** Covers the owner by `customer_id`, the owner by email (case-insensitive), a stranger, and a phone-auth user with JWT `email: ""` or none, against bookings with NULL / `''` / blank email.
  ...
  - `jobs_interview_rooms` / `jobs_offer_letters` (and their events tables) are service-role-only by design. anon/authenticated hold **no** table grants (`permission denied`, the SEC-HARDEN-03 convention), and the single policy is scoped `to service_role`.
...
- **Shadow limitation (verifier C, round 3).** The catch-up replays pre-snapshot baseline files over the 06-11 snapshot, and `CREATE OR REPLACE` there reverts prod function hotfixes that exist in **no** repo file: `user_addresses_enforce_default()` (prod has a `pg_trigger_depth() > 1` guard), `is_platform_staff()`, `studio_seed_welcome_message()` and `update_thread_unread_counts()`. None touches the FIX-01 chains or the security gate. It does mean a fresh environment built from the repo gets the older trigger, so those prod bodies should be committed as a migration (follow-up).
- **Pre-existing escalation proven (verifier A, round 3):** a profile-less authenticated user self-inserted `role='staff'` and gained `is_staff_in('care')` (0 → 1 foreign claims visible). This is not introduced by FIX-01, and it's recorded as a platform prerequisite in §4, with G10 added to §6.3.
