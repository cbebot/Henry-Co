...
| `services` (V3-49) | `services_catalog_viewed`, `services_service_viewed`, `services_booking_started` | `henry.services.catalog.viewed`, `henry.services.service.viewed`, `henry.services.booking.started` |
...
| `studio agency` (SA-2/SA-3 build orchestration) | `build_job_created`, `build_transitioned`, `build_deployed`, `build_escalated`, `build_decision_queued`, `build_client_reviewed`, `build_aftercare_scheduled` | `henry.studio.build.job_created`, `henry.studio.build.transitioned`, `henry.studio.build.deployed`, `henry.studio.build.escalated`, `henry.studio.build.decision_queued`, `henry.studio.build.client_reviewed`, `henry.studio.build.aftercare_scheduled` |
| `studio operator` (SA-4 Owner-AI operator) | `operator_tick`, `operator_proposal_raised`, `operator_escalated`, `operator_action` | `henry.studio.operator.tick`, `henry.studio.operator.proposal_raised`, `henry.studio.operator.escalated`, `henry.studio.operator.action` |
...
| `risk` (V3-40 predictive fraud & risk) | `risk_entity_scored` (per-batch, per-tier counts), `risk_enforcement_held`, `risk_enforcement_frozen`, `risk_enforcement_released`, `risk_staff_overrode` (staff one-taps — the system can only flag), `risk_model_promoted`, `risk_model_rolled_back` (governed lifecycle); plus `sensitive_action_risk_held` (rides the `auth` domain — the neutral customer-facing hold surfaces at the sensitive-action gate) | `henry.risk.entity.scored`, `henry.risk.enforcement.held`, `henry.risk.enforcement.frozen`, `henry.risk.enforcement.released`, `henry.risk.staff.overrode`, `henry.risk.model.promoted`, `henry.risk.model.rolled_back`, `henry.auth.sensitive_action.risk_held` — payloads carry entity ids + tiers + counts only, never PII, never a raw score, never a provider/model name |
