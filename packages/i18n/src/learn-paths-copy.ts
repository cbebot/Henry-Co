...
const LEARN_PATHS_COPY_EN: LearnPathsCopy = {
  ...
  hero: {
    ...
    body:
      "Each path lists the courses in order. Complete them one at a time; your progress still lives in your Henry Onyx account so you can pause and return whenever you need.",
...
const LEARN_PATHS_COPY_FR: DeepPartial<LearnPathsCopy> = {
  ...
  hero: {
    ...
    body:
      "Chaque parcours énumère les cours dans l’ordre. Suivez-les un à un ; votre progression reste dans votre compte Henry Onyx, vous pouvez donc mettre en pause et reprendre quand vous le souhaitez.",
...
const LEARN_PATHS_COPY_ES: DeepPartial<LearnPathsCopy> = {
  ...
  hero: {
    ...
    body:
      "Cada itinerario muestra los cursos en orden. Complétalos uno a uno; tu progreso permanece en tu cuenta de Henry Onyx para que puedas pausar y volver cuando lo necesites.",
...
const LEARN_PATHS_COPY_PT: DeepPartial<LearnPathsCopy> = {
  ...
  hero: {
    ...
    body:
      "Cada trilha apresenta os cursos por ordem. Conclua-os um de cada vez; o seu progresso continua na sua conta Henry Onyx, por isso pode pausar e retomar sempre que precisar.",
...
const LEARN_PATHS_COPY_AR: DeepPartial<LearnPathsCopy> = {
  ...
  hero: {
    ...
    body:
      "كلّ مسار يعرض الدورات بالترتيب. أكملها واحدةً تلو الأخرى؛ يبقى تقدّمك محفوظًا في حساب Henry Onyx، فتستطيع التوقّف والعودة متى احتجت.",
...
const LEARN_PATHS_COPY_DE: DeepPartial<LearnPathsCopy> = {
  ...
  hero: {
    ...
    body:
      "Jeder Pfad listet die Kurse in der richtigen Reihenfolge auf. Schließen Sie sie einen nach dem anderen ab; Ihr Fortschritt bleibt in Ihrem Henry Onyx-Konto erhalten, sodass Sie pausieren und jederzeit wieder einsteigen können.",
...
const LEARN_PATHS_COPY_IT: DeepPartial<LearnPathsCopy> = {
  ...
  hero: {
    ...
    body:
      "Ogni percorso elenca i corsi in ordine. Completali uno alla volta; i tuoi progressi restano nel tuo account Henry Onyx, così puoi sospendere e riprendere quando vuoi.",
...
const LEARN_PATHS_COPY_ZH: DeepPartial<LearnPathsCopy> = {
  ...
  hero: {
    ...
    body:
      "每条路径都按顺序列出对应的课程。一次完成一门;你的进度始终保存在 Henry Onyx 账户里,随时都可以暂停或继续。",
    ctaPrimary: "浏览单门课程",
    ctaSecondary: "证书项目",
  },
  aside: {
...
const LEARN_PATHS_COPY_HI: DeepPartial<LearnPathsCopy> = {
  ...
  hero: {
    ...
    body:
      "हर पथ कोर्स को क्रम में दिखाता है। उन्हें एक-एक करके पूरा करें; आपकी प्रगति आपके Henry Onyx खाते में सुरक्षित रहती है, इसलिए आप जब चाहें रुक सकते हैं और लौट सकते हैं।",
...
const LEARN_PATHS_COPY_IG: DeepPartial<LearnPathsCopy> = {
  ...
  hero: {
    ...
    body:
      "Ụzọ ọ bụla na-edepụta nkuzi n’usoro. Mechaa ha otu n’otu; ọganihu gị ka nọ n’akaụntụ Henry Onyx gị, ya mere ị nwere ike ịkwụsịtụ ma laghachi mgbe ọ bụla ọ dị gị mkpa.",
...
const LEARN_PATHS_COPY_YO: DeepPartial<LearnPathsCopy> = {
  ...
  hero: {
    ...
    body:
      "Ọ̀nà kọ̀ọ̀kan ń tò àwọn ẹ̀kọ́ ní àtẹ̀léǹtẹ̀lé. Pari wọn lọ́kọ̀ọ̀kan; ìlọsíwájú rẹ wà nínú àkáùntì Henry Onyx rẹ, nítorí ó lè dáwọ́dúró kí ó sì padà sí i nígbàkígbà tí ó bá fẹ́.",
...
const LEARN_PATHS_COPY_HA: DeepPartial<LearnPathsCopy> = {
  ...
  hero: {
    ...
    body:
      "Kowace hanya tana lissafa kwasa-kwasai cikin tsari. Cika su ɗaya bayan ɗaya; ci gabanku yana zaune a asusunku na Henry Onyx, don haka za ku iya dakatarwa kuma ku dawo a duk lokacin da kuke buƙata.",
