...
export function humanizeAssistantText(text: string): string {
  return String(text ?? "")
    // Drop a dash that OPENS a line (a bulletiness tell) first, before the clause rule below
    // could turn it into a stray leading comma.
    .replace(/^[ \t]*[—–][ \t]*/gm, "")
    // A dash used to join clauses (spaces on both sides) becomes a comma: the punctuation a
