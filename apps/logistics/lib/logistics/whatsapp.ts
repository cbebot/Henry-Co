...
export type LogisticsWhatsAppResult = {
  ok: boolean;
...
async function sendViaTwilio(to: string, body: string): Promise<LogisticsWhatsAppResult> {
...
async function sendViaMeta(to: string, body: string): Promise<LogisticsWhatsAppResult> {
...
export async function sendLogisticsWhatsAppText(input: { phone?: string | null; body: string }) {
  ...
  if (!phone) {
    return {
      ...
      messageId: null,
      provider: null,
    } satisfies LogisticsWhatsAppResult;
  }
