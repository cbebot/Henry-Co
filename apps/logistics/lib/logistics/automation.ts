...
export async function runLogisticsAutomationCron() {
  ...
  for (const shipment of snapshot.shipments) {
    ...
    await admin.from("logistics_events").insert({
      ...
      description: `No shipment update for about ${Math.round(idleHours)} hours; flagged for operator review.`,
      actor_user_id: null,
    ...
    if (!openIssue) {
      await admin.from("logistics_issues").insert({
        ...
        summary: "Delivery update in progress",
        details: `We are confirming the latest status of this shipment and will update you shortly.`,
