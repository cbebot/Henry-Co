...
export function briefDomainSummary(brief: Pick<StudioBrief, "domainIntent"> | null | undefined): {
  ...
  if (di.path === "have") {
    return {
      ...
      body: di.desiredLabel
        ...
        : "You’ll use an existing domain at launch. Henry & Co. walks you through DNS when the site is ready.",
    };
  }
  ...
  if (di.path === "later") {
    return {
      title: "Web address (domain)",
      body: "You asked to decide the domain later with Henry & Co. We’ll shortlist options and confirm availability before anything is purchased.",
    };
  }
