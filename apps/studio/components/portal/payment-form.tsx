...
function reasonToMessage(
  ...
  errors: StudioPortalCopy["paymentForm"]["errors"],
): string {
  switch (reason) {
    case "missing_fields":
    ...
    case "duplicate_submission":
      return errors.duplicateSubmission;
    case "unauthorised":
...
export function PaymentForm({
  ...
  function validateAndSetFile(next: File | null) {
    ...
    if (!ACCEPT_MIME.includes(next.type)) {
      setState({ kind: "error", message: reasonToMessage("invalid_file_type", pf.errors) });
      return;
    }
    if (next.size > MAX_BYTES) {
      setState({ kind: "error", message: reasonToMessage("file_too_large", pf.errors) });
      return;
    }
    setFile(next);
    setState({ kind: "idle" });
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      setState({ kind: "error", message: reasonToMessage(result.reason, pf.errors) });
    });
  }
    ...
    return (
      <div className="portal-card-elev p-6 sm:p-8 text-center">
        ...
        <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-[var(--studio-ink-soft)]">
          {pf.successBody(invoiceNumber, amountLabel, state.reference)}
        </p>
    ...
    <form onSubmit={handleSubmit} className="portal-card-elev space-y-5 p-5 sm:p-7">
      ...
      <div>
        <label htmlFor="payment-reference" className="block text-[13px] font-semibold text-[var(--studio-ink)]">
          {pf.referenceLabel}<span className="ml-1 text-[#ff8f8f]">*</span>
        </label>
        <input
          ...
          placeholder={pf.referencePlaceholder}
      ...
      <div>
        <label className="block text-[13px] font-semibold text-[var(--studio-ink)]">
          {pf.proofLabel}<span className="ml-1 text-[#ff8f8f]">*</span>
        </label>
          ...
          <div className="mt-2 flex items-start gap-3 rounded-2xl border border-[var(--studio-line-strong)] bg-[rgba(255,255,255,0.04)] p-3 sm:p-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 text-[12px] text-[var(--studio-ink-soft)]">
                {bytesLabel(file.size)} · {file.type || pf.unknownType}
              </div>
            </div>
            <button
              type="button"
      ...
      <div>
        <label htmlFor="payment-notes" className="block text-[13px] font-semibold text-[var(--studio-ink)]">
          {pf.notesLabel} <span className="text-[var(--studio-ink-soft)]">{pf.notesOptional}</span>
        </label>
        <textarea
          id="payment-notes"
          name="notes"
