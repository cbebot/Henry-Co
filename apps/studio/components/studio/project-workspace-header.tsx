...
export async function ProjectWorkspaceHeader() {
  ...
  const copy = getStudioProjectCopy(locale).header;
  ...
  const chipUser = viewer.user
    ? {
        displayName: viewer.user.fullName || viewer.user.email || copy.yourAccountFallback,
      ...
      <div className="mx-auto flex max-w-[88rem] flex-wrap items-center justify-between gap-4 px-5 py-4 sm:px-8 lg:px-10">
        <div className="flex flex-wrap items-center gap-4">
          <Link href="/" className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm font-semibold text-[var(--studio-ink)]">{copy.clientWorkspace}</div>
            </div>
          </Link>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <PublicAccountChip
            ...
            menuItems={[
              { label: copy.menuViewInAccount, href: `${accountUrl}?ref=studio-project` },
              { label: copy.menuStartAnother, href: "/request" },
              { label: copy.menuStudioHome, href: "/" },
            ]}
          />
        </div>
      </div>
