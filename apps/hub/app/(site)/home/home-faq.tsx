...
export function HomeFaq({ copy, faqs }: HomeFaqProps) {
  ...
  return (
    <section id="questions" className="relative scroll-mt-24 border-t border-white/10">
      <motion.div
      ...
      >
        ...
        <motion.div variants={m.stagger} className="mt-12 border-t border-white/10 sm:mt-16">
          {items.map((item) => (
            <motion.div key={item.q} variants={m.reveal}>
              <details name="hub-faq" className="group border-b border-white/10">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-6 rounded-sm py-6 outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-white/70 [&::-webkit-details-marker]:hidden">
                  <span className="text-base font-medium tracking-tight text-white/85 transition-colors group-open:text-white sm:text-lg">
                    {item.q}
                  </span>
                  <ChevronDown
                    aria-hidden
                    className="h-5 w-5 shrink-0 text-white/35 transition-transform duration-300 group-open:rotate-180 group-open:text-[color:var(--accent)]"
                  />
                </summary>
                <div className="max-w-prose pb-7 text-sm leading-relaxed text-white/60 sm:text-base">
                  {item.a}
                </div>
              </details>
            </motion.div>
