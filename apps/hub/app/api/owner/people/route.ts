...
export async function POST(request: Request) {
  ...
  return withOwnerMutationContext(
    ...
    async () => {
      ...
      if (body.id) {
        const { data } = await admin.from("company_people").select("*").eq("id", body.id).maybeSingle();
      ...
      await writeOwnerAudit({
        action: body.id ? "owner.staff.person.update" : "owner.staff.person.create",
        entityType: "company_person",
        entityId: (beforeRecord?.id as string | undefined) ?? (body.id as string | undefined) ?? null,
        oldValues: beforeRecord,
        newValues: payload,
        division: normalized.division_slug ?? "hub",
      });
