...
export type LearnWhatsAppResult = {
  ok: boolean;
...
async function sendViaTwilio(to: string, body: string): Promise<LearnWhatsAppResult> {
...
async function sendViaMeta(to: string, body: string): Promise<LearnWhatsAppResult> {
...
export async function sendLearnWhatsAppText(input: {
  phone?: string | null;
  body: string;
}) {
  ...
  if (!phone) {
    return {
      ok: false,
      status: "skipped",
      reason: "A valid learner phone number is not available for WhatsApp delivery.",
      messageId: null,
      provider: null,
