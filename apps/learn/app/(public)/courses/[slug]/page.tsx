    ...
    <main className="mx-auto max-w-[92rem] px-5 py-14 sm:px-8 xl:px-10">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              {!canStart && viewer.user ? (
                <form action={enrollInCourseAction}>
                  ...
                  <button className="learn-button-primary rounded-full px-5 py-3 text-sm font-semibold" type="submit">
                    {course.accessModel === "paid" ? "Reserve seat" : "Enroll now"}
                  </button>
                </form>
              ) : null}
              {!viewer.user ? <ActionLink href="/login" label="Sign in to enroll" variant="secondary" /> : null}
              {viewer.user ? (
                <form action={toggleSavedCourseAction}>
                  <input type="hidden" name="courseId" value={course.id} />
                  <button className="learn-button-secondary rounded-full px-5 py-3 text-sm font-semibold" type="submit">
                    {saved ? "Saved" : "Save course"}
                  </button>
                </form>
              ) : null}
            </div>
            {enrollment?.status === "awaiting_payment" ? <p className="mt-4 text-sm text-amber-200">Payment confirmation is still pending. The course will unlock inside your learner dashboard after confirmation.</p> : null}
          </div>
        </div>
      </section>
      ...
      {related.length > 0 ? (
        <section className="mt-10">
          <LearnSectionIntro kicker="Related Learning" title="Continue inside the same capability lane." body="These related courses are part of the same academy category and preserve the same calm, progress-first product standard." />
