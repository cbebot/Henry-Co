...
export const metadata = { title: "How it works - HenryCo Learn" };
...
export default async function AcademyPage() {
  ...
  return (
    <main className="mx-auto max-w-[92rem] px-5 py-14 sm:px-8 xl:px-10">
      <LearnSectionIntro
        kicker="How HenryCo Learn works"
        title="From “interested” to “I finished”—in plain steps."
        body="You don’t need to be technical. Choose a program, sign in with your HenryCo account, learn in order, and—when the course includes it—pass a short assessment to unlock your certificate. Managers can also assign training; you’ll see both in one place."
      />

      <div className="mt-10 grid gap-5 lg:grid-cols-3">
        <LearnPanel className="rounded-[2rem]">
          <h3 className="text-2xl font-semibold tracking-[-0.04em] text-[var(--learn-ink)]">1. Discover</h3>
        ...
        <LearnPanel className="rounded-[2rem]">
          <h3 className="text-2xl font-semibold tracking-[-0.04em] text-[var(--learn-ink)]">2. Enroll & learn</h3>
          <p className="mt-3 text-sm leading-7 text-[var(--learn-ink-soft)]">
        ...
        <LearnPanel className="rounded-[2rem]">
          <h3 className="text-2xl font-semibold tracking-[-0.04em] text-[var(--learn-ink)]">3. Prove & keep</h3>
          <p className="mt-3 text-sm leading-7 text-[var(--learn-ink-soft)]">
      ...
      <div className="mt-10 grid gap-5 lg:grid-cols-2 xl:grid-cols-4">
        ...
        <LearnPanel className="rounded-[2rem]">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--learn-ink-soft)]">Courses</p>
          <p className="mt-3 text-4xl font-semibold tracking-[-0.05em] text-[var(--learn-ink)]">{academy.courses.length}</p>
        </LearnPanel>
        <LearnPanel className="rounded-[2rem]">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--learn-ink-soft)]">Paths</p>
          <p className="mt-3 text-4xl font-semibold tracking-[-0.05em] text-[var(--learn-ink)]">{academy.paths.length}</p>
        </LearnPanel>
        <LearnPanel className="rounded-[2rem]">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--learn-ink-soft)]">Instructors</p>
      ...
      <div className="mt-10 grid gap-5 xl:grid-cols-[1.05fr,0.95fr]">
        <LearnPanel className="learn-mesh rounded-[2rem] p-7 sm:p-8">
          <p className="learn-kicker">Your dashboard</p>
          <h3 className="mt-4 text-3xl font-semibold tracking-[-0.05em] text-[var(--learn-ink)]">
            Keep learning and admin in the right place.
          </h3>
          <p className="mt-4 max-w-2xl text-sm leading-8 text-[var(--learn-ink-soft)]">
            The <strong className="font-semibold text-[var(--learn-ink)]">course room</strong> on HenryCo Learn is where you read lessons, submit quizzes, and download certificates. Your{" "}
            <strong className="font-semibold text-[var(--learn-ink)]">HenryCo account</strong> shows the big picture—active courses, saved picks, assignments, billing, and teaching status—in one calm overview.
          </p>
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <a href={getAccountLearnUrl()} className="learn-button-secondary rounded-full px-5 py-3 text-sm font-semibold">
              Open Learn in account
            </a>
          </div>
        </LearnPanel>

        <LearnPanel className="rounded-[2rem]">
          <p className="learn-kicker">Want to teach?</p>
          <h3 className="mt-4 text-3xl font-semibold tracking-[-0.05em] text-[var(--learn-ink)]">
            We take instructor applications seriously.
          </h3>
