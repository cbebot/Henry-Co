...
export default async function OwnerCoursesPage() {
  ...
  return (
    <LearnWorkspaceShell
      kicker={t("Courses")}
      title={t("Manage the course catalog and internal program definitions.")}
      description={t("The course editor writes directly into the live academy store so marketing pages, learner dashboards, and internal assignments stay in sync.")}
      nav={ownerNav("/owner/courses", t)}
    >
      <LearnPanel className="rounded-[2rem]">
        <form action={saveCourseDefinitionAction} className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <input name="title" placeholder={t("Course title")} className="learn-input rounded-2xl px-4 py-3" required />
          <input name="subtitle" placeholder={t("Subtitle")} className="learn-input rounded-2xl px-4 py-3" required />
          ...
          <textarea name="summary" placeholder={t("Short summary")} className="learn-textarea rounded-2xl px-4 py-3 md:col-span-2 xl:col-span-3" rows={3} />
          <textarea name="description" placeholder={t("Long description")} className="learn-textarea rounded-2xl px-4 py-3 md:col-span-2 xl:col-span-3" rows={5} />
          <input name="tags" placeholder={t("seller, trust, launch")} className="learn-input rounded-2xl px-4 py-3 md:col-span-2 xl:col-span-1" />
          <input name="prerequisites" placeholder={t("Basic catalog prepared")} className="learn-input rounded-2xl px-4 py-3 md:col-span-2 xl:col-span-1" />
          <input name="outcomes" placeholder={t("Launch with trust, reduce moderation loops")} className="learn-input rounded-2xl px-4 py-3 md:col-span-2 xl:col-span-1" />
          ...
          <label className="flex items-center gap-3 rounded-2xl border border-[var(--learn-line)] px-4 py-3 text-sm text-[var(--learn-ink)]">
            ...
            {t("Certification course")}
          </label>
          <div className="md:col-span-2 xl:col-span-3">
            <PendingSubmitButton pendingLabel={t("Saving course details...")}>{t("Save course")}</PendingSubmitButton>
          </div>
        </form>
      </LearnPanel>
