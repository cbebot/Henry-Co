...
export default async function JobsPage({
  ...
  const primaryCta = viewer.user
    ? { label: t("My applications"), href: "/candidate/applications" }
    : { label: t("Join free to apply"), href: getSharedAccountSignupUrl("/candidate/profile") };

  const secondaryCta = { label: t("How hiring works"), href: "/help" };
  ...
  const matchingLabel =
    jobs.length >= ROLE_COUNT_THRESHOLD
      ? t("Matching now")
      : `${t("Verified roles")} · ${jobs.length} ${t("live now")}`;
  const matchingDetail =
    jobs.length >= ROLE_COUNT_THRESHOLD
      ? hasQuery
        ? t("Roles that fit the filters above.")
        : t("Everything live on the board right now.")
      : t("Each role passes employer review before it goes live.");
  ...
  const heroFacts = [
    ...
    },
    {
      ...
      detail: t("Teams that have passed our employer review."),
    },
    {
      icon: Sparkles,
    ...
    {
      icon: Timer,
      label: t("With response time"),
      value: responsiveRoles,
      detail: t("Posts where the employer shared an expected reply window."),
    },
  ];
      ...
      <div className="mx-auto max-w-7xl space-y-12 px-4 py-12 sm:px-6 lg:px-8">
        <section>
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1.15fr)_minmax(280px,0.85fr)] lg:items-end">
            <div>
              <p className="jobs-kicker">{t("Find your next role")}</p>
              ...
              <p className="mt-6 max-w-2xl text-sm leading-7 text-[var(--jobs-muted)]">
                {t("Browsing is always free. We only ask you to sign in when you save a role or apply, so your shortlist and applications stay private and attached to the right person.")}
              </p>
            </div>
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,280px)_1fr] lg:gap-12">
          <aside className="space-y-12 lg:sticky lg:top-28 lg:self-start">
            <div>
              ...
              >
                {t("Clear filters · all jobs")}
              </Link>
            </div>

            <div>
              <p className="jobs-kicker text-[10.5px] uppercase tracking-[0.22em]">{t("Hiring now")}</p>
              <ul className="mt-4 divide-y divide-[var(--jobs-line)] border-y border-[var(--jobs-line)]">
                {employers
          ...
          <section className="space-y-6">
            <div className="flex flex-col gap-3 border-b border-[var(--jobs-line)] pb-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="jobs-kicker text-[10.5px] uppercase tracking-[0.22em]">{t("Results")}</p>
                <h2 className="mt-2 text-xl font-semibold tracking-tight text-[var(--jobs-ink)] sm:text-2xl">
                  {jobs.length === 0
                    ? t("Nothing matched yet")
                    : `${jobs.length} ${jobs.length === 1 ? t("role") : t("roles")} ${hasQuery ? t("match your search") : t("are live")}`}
                </h2>
                <p className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-[var(--jobs-muted)]">
                  <span>{verifiedOnly ? t("Verified employers only") : t("All employers")}</span>
                  <span className="text-[var(--jobs-line)]">·</span>
                  <span>{internalOnly ? t("Internal roles only") : t("Internal and external")}</span>
                </p>
              </div>
              {jobs.length > 0 ? (
                <Link
                ...
                >
                  {t("Why trust matters")} <ArrowRight className="h-4 w-4" />
                </Link>
              ) : null}
            </div>
            ...
            {jobs.length === 0 ? (
              <EmptyState
                kicker={t("Try a small change")}
                title={t("We couldn’t find roles with that exact mix.")}
                body={t("Widen your search, remove one filter, or try a category from the left. Verified-only and internal-only toggles are the most common reason lists go quiet.")}
                action={
                  <Link
                    href="/jobs"
                    className="jobs-button-primary inline-flex items-center gap-2 rounded-full px-5 py-3 text-sm font-semibold"
                  >
                    {t("Show all open roles")}
