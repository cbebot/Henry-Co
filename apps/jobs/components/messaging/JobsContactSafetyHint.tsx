 ...
 * draft looks like it carries off-platform contact details (The Onyx Line,
 * WS-5 — jobs).
 *
 * It runs the same `contactSafety` verdict the server uses BEFORE persist, so
 * the candidate/employer learns up-front that phone numbers, emails, and
 * off-platform links can't be sent — turning a hard block into a calm, honest
 * nudge rather than a surprise rejection.
 *
 * Safe by construction: it renders ONLY the generic reassurance copy. It never
 * echoes the matched contact (no `maskedText`, no `patterns`) back to the
 * surface. AA: warm ink-on-paper using the jobs tokens, brass only as a hairline
 * accent — never white-on-gold.
 */
export function JobsContactSafetyHint({ text }: { text: string }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        borderLeft: "3px solid var(--jobs-brass)",
        background: "color-mix(in srgb, var(--jobs-brass) 8%, transparent)",
        color: "var(--jobs-ink)",
        fontSize: "0.82rem",
        lineHeight: 1.45,
      }}
    >
      ...
      <span>{body}</span>
      <span style={{ color: "var(--jobs-muted)", flexBasis: "100%" }}>
        {t(
          "Keep the conversation on Henry Onyx — phone numbers, emails, and off-platform links can't be sent. This is how we protect everyone in the hiring process.",
        )}
      </span>
      <button
        type="button"
