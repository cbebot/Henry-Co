  ...
  return (
    <div
      ...
      className={`acct-signal-card pointer-events-auto w-full max-w-[26rem] origin-bottom rounded-[1.25rem] border bg-[var(--acct-bg-elevated)] shadow-[0_18px_44px_rgba(17,24,39,0.18)] transition will-change-transform ${
        isHigh
          ? "border-[var(--acct-gold)]/30 ring-1 ring-[var(--acct-gold)]/15"
          : "border-[var(--acct-line)]"
      }`}
    ...
    >
      <div className="flex items-start gap-3 px-3.5 py-3 sm:gap-4 sm:px-4">
        ...
        <button
          type="button"
          onClick={onActivate}
          className="acct-signal-activate min-w-0 flex-1 rounded-xl text-start outline-none transition focus-visible:ring-2 focus-visible:ring-[var(--acct-gold)]/35"
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {isHigh ? (
              <span className="rounded-full bg-[var(--acct-red-soft)] px-2 py-0.5 text-[0.6rem] font-semibold uppercase tracking-[0.12em] text-[var(--acct-red)]">
                {t("Priority")}
              </span>
            ) : null}
          </div>
        ...
        <button
          ...
          className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[var(--acct-muted)] transition hover:bg-[var(--acct-surface)] hover:text-[var(--acct-ink)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--acct-gold)]/35"
        >
