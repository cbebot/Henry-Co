  ...
  try {
    ...
    if (!dedicatedInsert.error && dedicatedInsert.data?.id) {
      ...
      await admin.from("customer_activity").insert({
        ...
        description: "Use the generated reference so the live payment rail can verify and release your balance.",
        amount_kobo: amountKobo,
        status: "pending_verification",
      ...
      await publishNotification({
        ...
        body: `Transfer NGN ${amountNaira.toLocaleString()} using the bank details on the next screen and keep the generated reference intact.`,
    ...
    await admin.from("customer_activity").insert({
      ...
      activity_type: "wallet_funding_requested",
      title: `Wallet funding request — NGN ${amountNaira.toLocaleString()}`,
      description: "Use the generated reference so the team and payment rail can confirm the funds.",
      amount_kobo: amountKobo,
      status: "pending_verification",
      reference_type: "wallet_funding_request",
    ...
    await publishNotification({
      ...
      severity: "info",
      title: "Funding request created",
      body: `Transfer NGN ${amountNaira.toLocaleString()} using the bank details shown and keep the generated reference intact.`,
