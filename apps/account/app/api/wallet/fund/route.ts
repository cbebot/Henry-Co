  ...
  try {
    ...
    if (!dedicatedInsert.error && dedicatedInsert.data?.id) {
      ...
      await admin.from("customer_activity").insert({
        ...
        description: "Keep the payment reference intact so the live rail can verify and release your balance.",
        amount_kobo: amountKobo,
        status: "pending_verification",
      ...
      await publishNotification({
        ...
        body: `Transfer NGN ${amountNaira.toLocaleString()} using the bank details on the next screen and keep the reference intact for automatic reconciliation.`,
    ...
    await admin.from("customer_activity").insert({
      ...
      activity_type: "wallet_funding_requested",
      title: `Wallet funding request — NGN ${amountNaira.toLocaleString()}`,
      description: "Keep the payment reference intact so the team can match the funds cleanly.",
      amount_kobo: amountKobo,
      status: "pending_verification",
      reference_type: "wallet_funding_request",
    ...
    await publishNotification({
      ...
      severity: "info",
      title: "Funding request created",
      body: `Transfer NGN ${amountNaira.toLocaleString()} using the bank details shown and keep the reference intact for automatic reconciliation.`,
