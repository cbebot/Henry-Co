  ...
  return (
    <div className="space-y-6 acct-fade-in">
      ...
      <HeroCard
        ...
        blurb={t("A read-only view of your subscription. Manage it from the workspace where you set it up.")}
        ctaPrimary={
          workspaceHref
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="acct-card p-5"><p className="acct-kicker">{t("About this subscription")}</p><div className="mt-4 space-y-3 text-sm leading-7 text-[var(--acct-muted)]"><p>{t("This is a read-only summary of your subscription. Manage it from the workspace where you set it up.")}</p><p>{workspaceHref ? t("Use the button above to open the workspace where you manage this subscription.") : t("Management for this subscription isn't available here yet.")}</p><p>{referenceKeys.length > 0 ? t("Related invoices and support requests appear here.") : t("We'll show related invoices and support requests here once we can match them to this subscription.")}</p></div></div>
      </div>
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="acct-card p-5"><div className="mb-4 flex items-center justify-between gap-3"><div className="flex items-center gap-2"><Receipt size={14} className="text-[var(--acct-muted)]" /><p className="acct-kicker">{t("Linked invoices & receipts")}</p></div><Link href="/invoices" className="text-xs font-medium text-[var(--acct-gold)] hover:underline">{t("All invoices")}</Link></div>{relatedInvoices.length === 0 ? <p className="text-sm text-[var(--acct-muted)]">{t("No invoices or receipts are linked to this subscription yet. If you were billed elsewhere, contact support and we'll connect the record.")}</p> : <div className="space-y-2">{relatedInvoices.map((invoice) => <Link key={String(invoice.id)} href={`/invoices/${invoice.id}`} className="flex items-center justify-between rounded-xl bg-[var(--acct-surface)] px-4 py-3 transition-colors hover:bg-[var(--acct-line)]"><div><p className="text-sm font-medium text-[var(--acct-ink)]">{String(invoice.description || invoice.invoice_no || t("Invoice"))}</p><p className="text-xs text-[var(--acct-muted)]">{invoice.status ? localizeHumanizedToken(t, invoice.status) : t("Status pending")}</p></div><div className="text-right"><p className="text-sm font-semibold text-[var(--acct-ink)]">{formatCurrencyAmount(Number(invoice.total_kobo || 0), String(invoice.currency || "NGN"), { unit: "kobo", locale })}</p><p className="text-[0.65rem] text-[var(--acct-muted)]">{invoice.created_at ? formatDate(String(invoice.created_at), { locale }) : t("Date pending")}</p></div></Link>)}</div>}</section>
