...
import { getProfile, getSubscriptionContext } from "@/lib/account-data";
...
export default async function SubscriptionDetailPage({ params }: { params: Promise<{ subscriptionId: string }> }) {
  ...
  const [context, profile] = await Promise.all([
    getSubscriptionContext(user.id, subscriptionId),
    getProfile(user.id),
  ]);
  if (!context) {
  ...
  const truth = resolveAccountLedgerCurrencyTruth(subscription, {
  ...
  return (
    <div className="space-y-6 acct-fade-in">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="acct-card p-5"><p className="acct-kicker">Ledger truth</p><div className="mt-4 space-y-3 text-sm leading-7 text-[var(--acct-muted)]"><p>This route is read-only. It only reflects subscription rows that a division has already written into `customer_subscriptions`.</p><p>{workspaceHref ? "Management is still owned by the division workspace. The account hub can deep-link you there, but it does not mutate subscription state directly." : "No direct management route has been published for this division inside the account hub yet, so there is no manage action to expose here."}</p><p>{truth.settlementMessage} Base ledger currency: {truth.baseCurrency}. {truth.exchangeRateSource ? `FX source: ${truth.exchangeRateSource}.` : "No live FX source is attached to this shared subscription row yet."}</p><p>{referenceKeys.length > 0 ? "Linked invoices and support threads below are shown only when the shared ledger uses one of this subscription's published reference keys." : "This subscription row has no published reference key beyond its own record, so invoice/support linkage depends entirely on future division-side sync quality."}</p></div></div>
      </div>
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="acct-card p-5"><div className="mb-4 flex items-center justify-between gap-3"><div className="flex items-center gap-2"><Receipt size={14} className="text-[var(--acct-muted)]" /><p className="acct-kicker">Linked invoices & receipts</p></div><Link href="/invoices" className="text-xs font-medium text-[var(--acct-gold)] hover:underline">All invoices</Link></div>{relatedInvoices.length === 0 ? <p className="text-sm text-[var(--acct-muted)]">No invoice or receipt row is linked to this subscription in the shared ledger yet. If billing happened elsewhere, the missing piece is division-side invoice publishing or shared reference alignment.</p> : <div className="space-y-2">{relatedInvoices.map((invoice) => { const linkedTruth = resolveAccountLedgerCurrencyTruth(invoice, { country: profile?.country as string | null | undefined, preferredCurrency: profile?.currency as string | null | undefined }); return <Link key={String(invoice.id)} href={`/invoices/${invoice.id}`} className="flex items-center justify-between rounded-xl bg-[var(--acct-surface)] px-4 py-3 transition-colors hover:bg-[var(--acct-line)]"><div><p className="text-sm font-medium text-[var(--acct-ink)]">{String(invoice.description || invoice.invoice_no || "Invoice")}</p><p className="text-xs text-[var(--acct-muted)]">{String(invoice.status || "Status pending")}{!linkedTruth.supportsNativeSettlement && linkedTruth.pricingCurrency !== linkedTruth.settlementCurrency ? ` · settles in ${linkedTruth.settlementCurrency}` : ""}</p></div><div className="text-right"><p className="text-sm font-semibold text-[var(--acct-ink)]">{formatCurrencyAmount(Number(invoice.total_kobo || 0), linkedTruth.pricingCurrency, { unit: "kobo", locale: linkedTruth.locale })}</p><p className="text-[0.65rem] text-[var(--acct-muted)]">{invoice.created_at ? formatDate(String(invoice.created_at)) : "Date pending"}</p></div></Link>; })}</div>}</section>
