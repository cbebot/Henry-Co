...
export default async function WalletFundingRequestPage({ params }: Props) {
  ...
  if (!request) {
    return (
      <div className="acct-empty py-20">
        <p className="text-sm text-[var(--acct-muted)]">{t("Funding request not found.")}</p>
  ...
  return (
    <div className="space-y-6 acct-fade-in">
      <RouteLiveRefresh />
      <PageHeader
        title={t("Funding Request")}
        description={t("Transfer details, proof status, and review progress for this wallet funding request.")}
        icon={Wallet}
        actions={
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_22rem]">
        <div className="space-y-6">
            ...
            <div className="grid gap-4 border-t border-[var(--acct-line)] p-5 sm:grid-cols-2">
              <div className="rounded-[1.4rem] bg-[var(--acct-surface)] p-4">
                <div className="flex items-start justify-between gap-2">
                  <p className="acct-kicker">{t("Bank")}</p>
                  {request.bank_name ? (
                    <CopyValueButton value={request.bank_name} label={copyLabel} copiedLabel={copiedLabel} />
                  ) : null}
                </div>
                <p className="mt-2 text-sm font-semibold text-[var(--acct-ink)]">{t(request.bank_name || "Pending")}</p>
              </div>
              <div className="rounded-[1.4rem] bg-[var(--acct-surface)] p-4">
                <div className="flex items-start justify-between gap-2">
                  <p className="acct-kicker">{t("Account name")}</p>
                  {request.account_name ? (
                    <CopyValueButton value={request.account_name} label={copyLabel} copiedLabel={copiedLabel} />
                  ) : null}
                </div>
                <p className="mt-2 text-sm font-semibold text-[var(--acct-ink)]">{t(request.account_name || "Pending")}</p>
              </div>
              <div className="rounded-[1.4rem] bg-[var(--acct-surface)] p-4 sm:col-span-2">
                <div className="flex items-start justify-between gap-2">
                  <p className="acct-kicker">{t("Account number")}</p>
                  {request.account_number ? (
                    <CopyValueButton value={request.account_number} label={copyLabel} copiedLabel={copiedLabel} />
                  ) : null}
                </div>
                <p className="mt-2 text-lg font-semibold tracking-[0.12em] text-[var(--acct-ink)]">
                  {request.account_number || t("Pending")}
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="acct-card p-5 sm:p-6">
            <p className="acct-kicker">{t("Funding steps")}</p>
              ...
              {[
                {
                  ...
                  detail: request.proof_url
                    ? t("We have a proof file tied to this transfer. Finance will confirm the payment against the bank reference.")
                    : t(
                        request.instructions ||
                          "Transfer funds using the account details shown above, then upload proof once the transfer is complete."
                      ),
                ...
                },
                {
                  ...
                  detail: request.proof_url
                    ? t("Proof is attached to this funding request.")
                    : t("Attach the receipt or transfer confirmation so the HenryCo team can confirm the payment."),
                  complete: Boolean(request.proof_url),
                },
                {
                  icon: Clock3,
                  label: t("Payment review"),
                  detail:
                    request.status === "completed" || request.status === "verified"
                      ? t("The request has been confirmed and the balance can now move into available funds.")
                      : t("The HenryCo team still needs to confirm the transfer before the funds appear in available balance."),
        ...
        <aside className="space-y-4">
          <section className="acct-card p-5">
            <p className="acct-kicker">{t("Transfer note")}</p>
            <p className="mt-3 text-sm leading-6 text-[var(--acct-muted)]">
              {request.note || t("No extra note was added for this request.")}
            </p>
          </section>

          <section className="acct-card p-5">
            <p className="acct-kicker">{t("Proof status")}</p>
            <p className="mt-3 text-sm font-semibold text-[var(--acct-ink)]">
              {request.proof_url ? t("Proof uploaded") : t("Awaiting proof")}
            </p>
            <p className="mt-2 text-sm leading-6 text-[var(--acct-muted)]">
              {request.proof_uploaded_at
                ? formatSurfaceTemplate(t("Uploaded {date}"), {
                    date: formatDateTime(request.proof_uploaded_at, locale),
                  })
                : t("Upload a receipt or PDF confirmation to keep the request moving.")}
            </p>
            {request.proof_url ? (
              <a
