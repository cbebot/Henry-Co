...
test.describe("V3-01 T2 — reauth round-trip restores draft", () => {
  test.fixme(
    "`expireSession(both)` helper requires SUPABASE_SERVICE_ROLE_KEY wiring — see setup.ts.",
    async ({ page }) => {
      await signIn(page);
      ...
      // henry_events should have both refresh_failed + reauth_succeeded
      // rows. The reauth_succeeded row only lands after slice 6b
      // wires the client-side persistEvent bridge.
    },
  );
});
